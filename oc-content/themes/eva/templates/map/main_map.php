...
var places = [<?php osc_reset_items(); osc_reset_latest_items();
	 if(osc_is_home_page()) { 
	$mapLimit = eva_map_num_ads() ;
	$mapSearch = Search::newInstance();
	$mapSearch->dao->select('*');
	$mapSearch->dao->from(DB_TABLE_PREFIX . 't_item');
	$mapSearch->dao->where('b_enabled', 1);
	$mapSearch->dao->where('b_active', 1);
	$mapSearch->dao->where('b_spam', 0);
	$mapSearch->dao->orderBy(DB_TABLE_PREFIX.'t_item.dt_pub_date', 'DESC');
	if($mapLimit != 0) { $mapSearch->dao->limit(0, $mapLimit);}
	$result = $mapSearch->dao->get();
    if($result === false) {
		return array();
    }
    $items  = $result->result();
	$items2 = Item::newInstance()->extendData($items);
	View::newInstance()->_exportVariableToView('items', $items2);
		while(osc_has_items()) { 
		//if (file_exists(osc_base_path() . 'oc-content/themes/eva/img/' . osc_item_category_id() . '.png')) {
			//$icon_url = osc_current_web_theme_url('img/').osc_item_category_id().'.png' ;
			//} else if (file_exists(osc_base_path() . 'oc-content/themes/eva/img/' . eva_category_root(osc_item_category_id()) . '.png')) {
			//$icon_url = osc_current_web_theme_url('img/').eva_category_root(osc_item_category_id()).'.png' ;
			//} else { $icon_url = osc_current_web_theme_url('img/').'pin.png' ; }
	//o.addListener
  ...
  <?php if($lat != ''){?>[<?php echo $lat; ?>,<?php echo $long; ?>,"<?php echo osc_esc_html(osc_item_title()); ?>",'<?php echo osc_format_date(osc_item_pub_date()); ?>','<?php echo osc_item_url(); ?>','<?php if( osc_count_item_resources() ) { ?><?php echo osc_resource_thumbnail_url(); ?><?php } else { ?><?php echo osc_current_web_theme_url('img/no_photo.gif') ; ?><?php } ?>','<?php echo osc_item_formated_price(); ?>'],<?php } ?>
	<?php } osc_reset_items(); osc_reset_latest_items(); ?><?php } ?>
];
