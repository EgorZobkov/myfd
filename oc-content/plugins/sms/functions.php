...
function sms_providers() {
  return 'demo,gatewayapi.com,twilio,textlocal.in,textlocal.com,budgetsms,way2sms,plivo,ringcaptcha,msg91,notify.lk,smsassistent.by';
}
  ...
  if(sms_param('verify_phone') == 1) {
    ...
    if($phone <> '') {
      ...
      if($user && isset($user['pk_i_id']) && $user['pk_i_id'] != osc_logged_user_id()) {
        osc_add_flash_error_message(sprintf(__('Phone number %s is already registered to another account, please use different number', 'sms'), $phone));
    ...
    if($phone_land <> '') {
      ...
      if($user && isset($user['pk_i_id']) && $user['pk_i_id'] != osc_logged_user_id()) {
        osc_add_flash_error_message(sprintf(__('Phone number %s is already registered to another account, please use different number', 'sms'), $phone_land));
...
function sms_send($phone_number, $message, $action, $provider = '') {
  $error = false;
  ...
  // SMS-ASSISTENT.BY
    ...
    $data = array('user' => sms_param('assistent_api_username'), 'password' => sms_param('assistent_api_pass'), 'sender' => sms_param('assistent_api_sender'), 'message' => $message, 'recipient' => $phone_number, 'validity_period' => 48);

/*
    $ch = curl_init('https://userarea.sms-assistent.by/api/v1.2/send_sms/plain');
...
*/
  ...
  // MSG91.COM
    ...
    $data = array('smshkey' => sms_param('msg91_api_key'), 'phone_numbers' => $phone_number, 'message' => urlencode($message), 'sender' => sms_param('msg91_sender_id'), 'route' => 4);
  ...
  $output = sms_provider_status($response, $provider);
...
function sms_provider_status($data, $provider = '') {
  $provider = ($provider <> '' ? $provider : sms_param('provider'));
  ...
  // TWILIO
    ...
    if($object->errorCode <> '' || (string)$object->errorMessage <> '') {
