  ...
  if(Params::getParam('smsAjax') == 1) {
    ...
    if(Params::getParam('step') == 1) {
      ...
      if($phone_number !== false) {
        ...
        $output = sms_send($phone_number, $message, 'VALIDATE_' . $type);
        ...
        if($output['status'] == 'ERROR' && $provider_alt <> '') {
          ...
          $output = sms_send($phone_number, $message, 'VALIDATE_' . $type, $provider_alt);
        ...
        if($output['status'] == 'ERROR') {
          ...
          echo json_encode(array('status' => 'OK', 'code' => (osc_is_admin_user_logged_in() ? $code : ($provider == 'demo' ? $code : '')), 'message' => ($provider == 'demo' ? sprintf(__('Проверка номера работает в демо режиме. Пожалуйста, введите в поле этот код: %s', 'sms'), $code) : '')));
        }
    ...
    if(Params::getParam('step') == 2) {
      ...
      if($phone_number !== false && $code <> '' && strlen($code) == 4) {
        ...
        if($status) {
          ModelSMS::newInstance()->updateVerification(
            array(
              's_phone_number' => $phone_number, 
              's_status' => 'VERIFIED'
            )
