...
<?php 
  if(im_param('only_logged') == 1 && !osc_is_web_user_logged_in()) {
    ...
    osc_add_flash_error_message( __('Пожалуйста, войдите в систему, отправлять мгновенные сообщения могут только аутентифицированные пользователи.', 'instant_messenger'));
  ...
  if( Params::getParam('item-id') <> '' && Params::getParam('item-id') > 0 ) {
    ...
    if(Params::getParam('im-action') == 'create_thread') {
      ...
      if(im_check_block($to_user_id, $from_user_email) == 0) {
        //osc_add_flash_error_message( __('Вы не можете отправить сообщение этому пользователю. Этот пользователь заблокировал связь с вами.', 'instant_messenger'));
...
<div class="im-html im-file-create-thread im-theme-<?php echo osc_current_web_theme(); ?>">
  <?php if(osc_is_web_user_logged_in() && $item['fk_i_user_id'] <> osc_logged_user_id() || !osc_is_web_user_logged_in()) { ?>
    <h2 class="im-head"><?php _e('Начать диалог', 'instant_messenger'); ?></h2>
    ...
    <form id="im-create-thread-form" name="im-create-thread-form" class="im-body im-form-validate" action="<?php echo osc_route_url( 'im-create-thread', array('item-id' => $item_id) ); ?>" method="POST" enctype="multipart/form-data">
      ...
      <div class="im-row">
        <div class="im-col-24">
          <label class="im-label" for="im-from-user-name"><?php _e('Ваше имя', 'instant_messenger'); ?></label>
      ...
      <div class="im-row">
        <div class="im-col-24">
          <label class="im-label" for="im-from-user-email"><?php _e('Ваша почта', 'instant_messenger'); ?></label>
      ...
      <div class="im-row">
        <div class="im-col-24">
          <label class="im-label" for="im-title"><?php _e('Заголовок', 'instant_messenger'); ?></label>
          <input type="text" class="im-input im-big" name="im-title" id="im-title" placeholder="<?php echo osc_esc_html(__('Заголовок сообщения', 'instant_messenger')); ?>" value="" />
        </div>
      </div>

      <div class="im-row">
        <div class="im-col-24">
          <label class="im-label" for="im-message"><?php _e('Сообщение', 'instant_messenger'); ?></label>
          <textarea name="im-message" id="im-message" class="im-textarea" placeholder="<?php echo osc_esc_html(__('Напишите здесь все подробности', 'instant_messenger')); ?>"></textarea>
        </div>
      </div>

      <button type="submit" class="im-button-green"><?php _e('Отправить', 'instant_messenger'); ?></button>

      <?php if(im_param('att_enable') == 1) { ?>
        <div class="im-attachment">
          <div class="im-att-box">
            <label class="im-status">
              <span class="im-wrap"><i class="fa fa-paperclip"></i> <span><?php _e('Добавить файл', 'instant_messenger'); ?></span></span>
    ...
    <?php if(count($threads) > 0) { ?>
      <div class="im-threads-exist im-body">
        <h3 class="im-head"><?php _e('Вы уже связывались с продавцом по этому объявлению, возможно, вы захотите продолжить существующий разговор', 'instant_messenger'); ?></h3>

        <?php foreach($threads as $t) { ?>
          <?php $time_diff = im_get_time_diff( $t['d_datetime'] ); ?>

          <a class="im-row im-has-tooltip-left" href="<?php echo osc_route_url( 'im-messages', array('thread-id' => $t['i_thread_id'], 'secret' => 'n') ); ?>" title="<?php _e('Открыть диалог', 'instant_messenger'); ?>">
            <div class="im-col-12 im-b im-title"><?php echo ( $t['s_title'] <> '' ? osc_highlight($t['s_title'], 40) : __('Нет сообщений', 'instant_messenger') ); ?></div>
            <div class="im-col-4 im-from-to"><?php echo ($t['i_from_user_id'] == osc_logged_user_id() ? __('от', 'instant_messenger') : __('от', 'instant_messenger')); ?> <strong><?php echo $t['s_to_user_name']; ?></strong></div>
            <div class="im-col-4 im-pms im-align-center"><?php echo $t['i_count'] . ' ' . ( $t['i_count'] == 1 ? __('', 'instant_messenger') : __('', 'instant_messenger') ); ?></div>
  ...
  <?php } else { ?>
    <div class="im-empty flashmessage flashmessage-warning"><?php _e('Вы не можете связаться с самим собой', 'instant_messenger'); ?></div>
  <?php } ?>
</div>
