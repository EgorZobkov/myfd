...
<?php 
  ...
  if($offer) {
    ...
    $t_title = sprintf(__('Новые запросы %s - %s', 'instant_messenger'), osc_highlight($item['s_title'], 50), $offer['i_price']/1000000 . $currency['s_description']);
  } else if($thread['s_title'] <> '') {
    $t_title = osc_highlight($thread['s_title'], 60);
  } else {
    $t_title =  __('Нет объектов', 'instant_messenger');
  }
  ...
  if(Params::getParam('del-message-id') <> '' && Params::getParam('del-message-id') > 0 && $message_delete == 1) {
    ...
    if($del_message['fk_i_thread_id'] == $thread['i_thread_id'] && $del_message['i_type'] == $type) {
      ...
      osc_add_flash_ok_message(__('Сообщение удалено', 'instant_messenger'));

      header('Location: ' . osc_route_url('im-messages', array('thread-id' => $del_message['fk_i_thread_id'], 'secret' => $secret)));
      exit;
    } else {
      osc_add_flash_error_message(__('Это не ваше сообщение, вы не можете удалить его!', 'instant_messenger'));
    }
  }
  ...
  if(Params::getParam('del-att-message-id') <> '' && Params::getParam('del-att-message-id') > 0 && Params::getParam('del-file-name') <> '') {
    ...
    if($del_message['fk_i_thread_id'] == $thread['i_thread_id'] && $del_message['i_type'] == $type) {
      @unlink(osc_content_path() . 'plugins/instant_messenger/download/' . Params::getParam('del-file-name'));
      ...
      osc_add_flash_error_message(__('Это не ваше сообщение, вы не можете удалить вложение к нему!', 'instant_messenger'));
    }
  }
...
<div class="im-html im-file-messages im-theme-<?php echo osc_current_web_theme(); ?>">
  <?php if((($thread['i_from_user_id'] == osc_logged_user_id() || $thread['i_to_user_id'] == osc_logged_user_id()) && osc_is_web_user_logged_in()) || ($secret == $thread['s_from_secret'] || $secret == $thread['s_to_secret'])) { ?>
    ...
    <div class="im-alt-head" style="display:none;">
      <div class="im-head2">
        ...
        <?php if($target_is_null) { ?>
          <em><?php echo sprintf(__('%s удалил эту тему, вы не можете ответить.', 'instant_messenger'), $thread_target_name); ?></em>
        <?php } ?>
          
        <?php if(im_param('remove_thread') == 1) { ?>
          <a href="<?php echo osc_route_url('im-thread-remove', array('thread-remove-id' => Params::getParam('thread-id'), 'secret' => $secret)); ?>" class="im-remove-thread" onclick="return confirm('<?php echo osc_esc_js(__('Вы уверены, что хотите удалить эту тему? Действие не может быть отменено!', 'instant_messenger')); ?>');" title="<?php echo osc_esc_html(__('Удалите эту тему, связанные с ней сообщения и вложения из своей учетной записи', 'instant_messenger')); ?>"><i class="fa fa-trash"></i><span><?php _e('Remove', 'instant_messenger'); ?></span></a>
        <?php } ?>
      </div>
      
      <?php if($last_seen <> '') { ?>
        <div class="im-subhead2"><?php echo sprintf(__('Был в сети %s', 'instant_messenger'), $last_seen); ?></div>
      <?php } ?>
    </div>
    ...
    <?php if($offer) { ?>
      <a href="<?php echo osc_route_url('mo-show-offers', array('offerId' => $thread['i_offer_id'])); ?>" class="im-row im-body im-offer">
        <div class="im-line"><?php echo sprintf(__('Запрос: %sx %s для %s%s', 'instant_messenger'), $offer['i_quantity'], $offer_item['s_title'], $offer['i_price']/1000000, $currency['s_description']); ?></div>
        <div class="im-line"><?php _e('Нажмите для уточнения', 'instant_messenger'); ?> <i class="fa fa-angle-double-right"></i></div>
      </a>
    <?php } ?>
    ...
    <?php if(count($messages) > 0) { ?>
      <div class="im-table im-messages im-body">
        ...
        <?php if(count($messages) > $show_last) { ?>
          <div class="im-show-older"><span><?php _e('Показать сообщения', 'instant_messenger'); ?></span></div>
        <?php } ?>
        ...
        <?php foreach($messages as $m) { ?>
          <?php 
            ...
            if((osc_is_web_user_logged_in() && (osc_logged_user_id() == $thread['i_from_user_id'] && $m['i_type'] == 0 || osc_logged_user_id() == $thread['i_to_user_id'] && $m['i_type'] == 1)) || ($secret == $thread['s_from_secret'] && $m['i_type'] == 0 || $secret == $thread['s_to_secret'] && $m['i_type'] == 1)) {
              ...
              if($m['i_type'] == 0) {
                $identify_name = __('Продавец', 'instant_messenger');
              } else {
                $identify_name = __('ПОкупатель', 'instant_messenger');
              }
            } 
          ...
          <div class="im-table-row<?php if($logged_is_owner) { ?> im-from<?php } ?><?php if(count($messages) - $i >= $show_last) { ?> hidden<?php } ?>" data-message-id="<?php echo $m['pk_i_id']; ?>">
            ...
            <div class="im-line im-name-top">
              ...
              <div class="im-col-12 im-date im-align-right im-i im-gray" title="<?php echo date('d/m/Y H:i:s', strtotime($m['d_datetime'])); ?>">
                ...
                <?php if($m['i_read'] == 1) { ?>
                  <i class="fa fa-check im-has-tooltip" title="<?php echo osc_esc_html(sprintf(__('%s уже прочитал это сообщение', 'instant_messenger'), ($m['i_type'] == 1 ? $thread['s_from_user_name'] : $thread['s_to_user_name']))); ?>"></i> 
                <?php } ?>
              </div>
            </div>
            ...
            <div class="im-line im-message-extra <?php if($m['s_file'] <> '' && $att_enable == 1) { ?>im-box-gray<?php } else { ?>im-box-empty<?php } ?>">
              <div class="im-col-10" class="im-align-left">
                <?php if($m['s_file'] <> '' && $att_enable == 1) { ?>
                  <a class="im-download" href="<?php echo osc_base_url(); ?>oc-content/plugins/instant_messenger/download/<?php echo $m['s_file']; ?>" target="_blank">
                    ...
                    <?php _e('Вложения', 'instant_messenger'); ?>
                  </a>
                <?php } ?>
              </div>
              ...
              <?php if($logged_is_owner) {?>
                <div class="im-col-14 im-align-right">
                  <?php if($m['s_file'] <> '' && $att_enable == 1) { ?>
                    <a class="im-hide" href="<?php echo osc_route_url('im-delete-attachment', array('thread-id' => $thread['i_thread_id'], 'del-att-message-id' => $m['pk_i_id'], 'del-file-name' => $m['s_file'], 'secret' => $secret)); ?>" onclick="return confirm('<?php echo osc_esc_js(__('Вы уверены, что хотите удалить вложение?', 'instant_messenger')); ?>?')"><span><?php _e('Remove file', 'instant_messenger'); ?></span><i class="fa fa-trash" style="display:none;"></i></a>
                  <?php } ?>
                </div>
              <?php } ?>
            </div>
            ...
            <?php if($logged_is_owner && $message_delete == 1) {?>
              <div class="im-del-mes-box">
                <a href="<?php echo osc_route_url('im-delete-message', array('thread-id' => $thread['i_thread_id'], 'del-message-id' => $m['pk_i_id'], 'secret' => $secret)); ?>" onclick="return confirm('<?php echo osc_esc_js(__('Вы уверены, что хотите удалить это сообщение', 'instant_messenger')); ?>?')"><i class="fa fa-trash"></i></a>
              </div>
            <?php } ?>
          </div>
    ...
    <?php } else { ?>
      <div class="im-empty flashmessage flashmessage-warning"><?php _e('У вас нет никаких сообщений', 'instant_messenger'); ?></div>
    <?php } ?>
    ...
    <?php if($target_is_null === false) { ?>
      <form id="im-message-form" class="im-row im-body im-form-validate" action="<?php echo osc_route_url('im-messages', array('thread-id' => $thread['i_thread_id'], 'secret' => $secret)); ?>" method="POST" enctype="multipart/form-data">
        ...
        <textarea name="im-message" id="im-message" class="im-textarea" placeholder="<?php echo osc_esc_js(__('Введите сообщение', 'instant_messenger')); ?>" required></textarea>

        <button type="submit" class="im-button-green"><?php _e('Напишите сообщение', 'instant_messenger'); ?></button>
        <button type="submit" class="im-button-green im-button-alt" style="display:none;width:10px;"><i class="fa fa-paper-plane"></i></button>

        <?php if($att_enable == 1) { ?>
          <div class="im-attachment">
            <div class="im-att-box">
              <label class="im-status">
                <span class="im-wrap"><i class="fa fa-paperclip"></i> <span><span class="im-def-text"><?php _e('Прикрепить файл', 'instant_messenger'); ?></span></span></span>
  ...
  <?php } else { ?>
    <div class="im-empty flashmessage flashmessage-warning"><?php _e('Это не ваша тема, вы не можете читать сообщения других пользователей!', 'instant_messenger'); ?></div>
  <?php } ?>
</div>
      ...
      if(response.length) {
        ...
        if(
          ...
          if(!$('.im-table.im-messages .im-table-row:last-child').hasClass('im-from') && $('.im-table.im-messages .im-table-row:last-child').attr('data-message-id') != lastMessageId) {
            ...
            PageTitleNotification.On('<?php echo osc_esc_js(__('У вас новое сообщение', 'instant_messenger')); ?>');
