  ...
  <div class="im-table im-block im-body">
    <div class="im-table-head">
      <div class="im-col-22 im-align-left">
        <strong><?php _e('Заблокированный юзер', 'instant_messenger'); ?></strong>
      </div>
      <?php if(count($blocks) > 0) { ?>
        <div class="im-col-2"><?php _e('Отменить', 'instant_messenger'); ?></div>
      <?php } ?>
    </div>
    ...
    <?php if(count($blocks) > 0) { ?>
      ...
      <?php } ?>
    <?php } else { ?>
      <div class="im-table-row im-empty im-align-center"><?php _e('Активных блокировок нет', 'instant_messenger'); ?></div>
    <?php } ?>
  </div>
<?php } ?>