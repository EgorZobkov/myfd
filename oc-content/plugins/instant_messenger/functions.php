  ...
  if(
    ...
    if(is_array($rmessages) && count($rmessages) > 0) {
      foreach($rmessages as $rm) {
        if(@$rm['s_file'] != '') {
          @unlink(osc_content_path() . 'plugins/instant_messenger/download/' . $rm['s_file']);
        }
      }
    }
...
function im_remove_message($id) {
  ...
  if(@$message['s_file'] != '') {
    @unlink(osc_content_path() . 'plugins/instant_messenger/download/' . $message['s_file']);
  }  
    ...
    if( $file['error'] == UPLOAD_ERR_OK) {
      if(in_array($extension, $allowed_extensions)) {
        if( $file_size < $max_file_size ) {
          if( move_uploaded_file(@$file['tmp_name'], osc_content_path() . 'plugins/instant_messenger/download/' . $file_name ) ) {
...
function im_get_time_diff( $time ) {
  ...
  if($time_diff < 2) {
    ...
    $time_diff_name = sprintf(__('%d Минут назад', 'instant_messenger'), $time_diff);
  } else if ($time_diff < 120) {
    $time_diff_name = sprintf(__('%d час назад', 'instant_messenger'), $time_diff_h);
  } else if ($time_diff < 1440) {
    $time_diff_name = sprintf(__('%d часов назад', 'instant_messenger'), $time_diff_h);
  } else if ($time_diff < 2880) {
    $time_diff_name = sprintf(__('%d день назад', 'instant_messenger'), $time_diff_d);
  } else if ($time_diff < 10080) {
    $time_diff_name = sprintf(__('%d дней назад', 'instant_messenger'), $time_diff_d);
  } else if ($time_diff < 20160) {
    $time_diff_name = sprintf(__('%d неделю назад', 'instant_messenger'), $time_diff_w);
  } else if ($time_diff < 43200) {
    $time_diff_name = sprintf(__('%d недель назад', 'instant_messenger'), $time_diff_w);
  } else if ($time_diff < 86400) {
    $time_diff_name = sprintf(__('%d месяц назад', 'instant_messenger'), $time_diff_m);
  } else {
    $time_diff_name = sprintf(__('%d месяцев назад', 'instant_messenger'), $time_diff_m);
  }
