...
function im_messages() {
  ...
  if( osc_is_web_user_logged_in() || im_param('link_reg_only') <> 1) {
    ...
    $html .= '<a href="' . osc_route_url( 'im-threads') . '">' . __('СООБЩЕНИЯ', 'instant_messenger') . '<span class="im-t-unread">' . $count .  '</span></a>';
    ...
    foreach( $threads as $t ) {
      ...
      $html .= '<a class="im-entry im-row ' . ($m_unread ? 'im-unread' : '') . '" href="' . osc_route_url('im-messages', array('thread-id' => $t['i_thread_id'], 'secret' => 'n')) . '" title="' . __('Нажмите, чтобы перейти к диалогу', 'instant_messenger') . '">';
        $html .= '<div class="im-col-6 im-img"><img src="' . $item_details['resource'] . '" /></div>';
        $html .= '<div class="im-col-18">';
          $html .= '<div class="im-row im-name">' . ($t['i_from_user_id'] == osc_logged_user_id() ? __('', 'instant_messenger') . ' ' . $t['s_to_user_name'] : __('', 'instant_messenger') . ' ' . $t['s_from_user_name']) . '</div>';
    ...
    if(count($threads) > 0) {
      $html .= '<div class="im-row im-show-all"><a href="' . osc_route_url( 'im-threads') . '">' . __('Показать все сообщения', 'instant_messenger') . '</a></div>';
    } else {
      if(osc_is_web_user_logged_in()) {
        $html .= '<div class="im-row im-show-all"><a href="' . osc_route_url( 'im-threads') . '">' . __('У вас нет новых сообщений', 'instant_messenger') . '</a></div>';
      } else {
        $html .= '<div class="im-row im-show-all"><a href="' . osc_route_url( 'im-threads') . '">' . __('Зайдите, чтобы увидеть сообщения', 'instant_messenger') . '</a></div>';
      }
    }
...
function im_delete_threads_item( $item_id ) {
  ...
  foreach($threads as $t) {
    ...
    foreach($messages_files as $m) {
      $file_to_delete = osc_content_path() . 'plugins/instant_messenger/download/' . $m['s_file'];
...
function im_user_menu_link(){
  if(osc_current_web_theme() == 'veronika' || osc_current_web_theme() == 'stela' || osc_current_web_theme() == 'starter' || (defined('USER_MENU_ICONS') && USER_MENU_ICONS == 1)) {
    echo '<li class="opt_instant_messenger"><a href="' . osc_route_url('im-threads') .'" ><i class="fa fa-envelope-o"></i> '.__('Сообщения', 'instant_messenger').'<span class="im-user-account-count im-count-' . im_count_unread(osc_logged_user_id()) . '">' . im_count_unread(osc_logged_user_id()) . '</a></li>';
  } else {
    echo '<li class="opt_instant_messenger"><a href="' . osc_route_url('im-threads') .'" >'.__('Сообщения', 'instant_messenger').'<span class="im-user-account-count im-count-' . im_count_unread(osc_logged_user_id()) . '">' . im_count_unread(osc_logged_user_id()) . '</span></a></li>';
  }
}
...
osc_add_route('im-threads', 'im-threads', '', osc_plugin_folder(__FILE__).'user/threads.php', $menu, 'im', 'thread', __('Threads', 'instant_messenger'));
...
osc_add_route('im-messages', 'im-messages/([0-9]+)/(.+)', 'user-messenger/{thread-id}/{secret}', osc_plugin_folder(__FILE__).'user/messages.php', $menu, 'im', 'message', __('Messages', 'instant_messenger'));
...
function im_contact_button( $item = NULL, $link_only = false ) {
  ...
  if(im_param('only_logged') == 1 && !osc_is_web_user_logged_in()) {
    if($link_only) {
      ...
      echo '<a href="' . osc_user_login_url() . '" class="btn-pink btn-full-width upcase"><span class="im-top">' . __('Написать', 'instant_messenger') . '</span><span class="im-bot">' . __('Only for logged in users', 'instant_messenger') . '</span><i class="fa fa-envelope"></i></a>';
      return true;
    }
    ...
    if($link_only) {
      ...
      echo '<a href="' . osc_route_url( 'im-create-thread', array('item-id' => $item_id) ) . '" class="btn-pink btn-full-width upcase"><span class="im-top">' . __('Написать', 'instant_messenger') . '</span></a>';
      return true; 
    }
