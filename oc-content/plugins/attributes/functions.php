...
function atr_generate_search_elem($a, $linked = false) {
  ...
  if($a['s_search_type'] == 'BOXED' && ($a['s_type'] == 'RADIO' || $a['s_type'] == 'CHECKBOX' || $a['s_type'] == 'CLR')) {
    $type = $a['s_type'];
    $subtype = 'BOXED';
  } else {
  ...
  if($linked === false) {
    ...
    $html .= '<label class="control-label inp-group__title" for="atr' . $a['pk_i_id'] . '">' . ($name <> '' ? $name : __('New attribute', 'attributes')) . '</label>';
      ...
      if(is_array($data) && count($data) > 0) {
        ...
        foreach($data as $d) {
          ...
          if(is_array($siblings) && count($siblings) > 0) {
            ...
            if($linked === false) {
              $html .= '<option value="">' . ($name <> '' ? sprintf(__('%s ...', 'attributes'), strtolower($name)) : __('Select value...', 'attributes')) . '</option>';
            }
  ...
  } else if($type == 'CHECKBOX' || $type == 'RADIO' || $type == 'CLR') {
    if(is_array($a['values']) && count($a['values']) > 0) {

      if($subtype == 'BOXED') {
        $html .= '<div class="atr-box" data-empty="' . osc_esc_html($name <> '' ? sprintf(__('%s ...', 'attributes'), strtolower($name)) : __('Select value ...', 'attributes')) . '"><div class="atr-holder">';
        ...
        if(!$found_val) {
          $html .= ($name <> '' ? sprintf(__('%s ...', 'attributes'), strtolower($name)) : __('Select value ...', 'attributes'));
        }
      ...
      if(($type == 'CHECKBOX' && ($a['s_search_type'] == '' || $a['s_search_type'] == 'BOXED')) || ($a['s_search_type'] == 'CHECKBOX')) {
        $html .= '<a href="' . atr_select_all_url($a) . '" class="atr-select-deselect atr-select-all mb-ajax-search-link" id="atr-select-all-' . $a['pk_i_id'] . '" ' . (atr_select_or_deselect($a) ? 'style="display:none;"' : '') . '>' . __('Отметить все', 'attributes') . '</a>';
        $html .= '<a href="' . atr_deselect_all_url($a) . '" class="atr-select-deselect atr-deselect-all mb-ajax-search-link" id="atr-deselect-all-' . $a['pk_i_id'] . '" ' . (!atr_select_or_deselect($a) ? 'style="display:none;"' : '') . '>' . __('Снять отметки', 'attributes') . '</a>';
      }
    ...
    if(@$atr[0] == 'atr' && @$atr[1] > 0) {
      ...
      if(isset($attribute['pk_i_id']) && $attribute['b_search'] == 1 && $attribute['b_enabled'] == 1) {
        ...
        if($cat_ok === true) {
          ...
          if($attribute['s_search_engine'] == 'OR' && ($type == 'SELECT' || $type == 'RADIO' || $type == 'CHECKBOX' || $type == 'CLR')) {
          ...
          } else if($type == 'SELECT' || $type == 'RADIO' || $type == 'CHECKBOX' || $type == 'CLR') {
            $value = ($value > 0 ? $value : null);
          
            if($type == 'SELECT' && ($search_type <> 'RADIO' && $search_type <> 'CHECKBOX' && $search_type <> 'CLR')) {
          ...
          if(($value <> '' || $attribute_value_id <> '' || @$from <> '' || @$to <> '') && !in_array($attribute_id, $processed)) {
            ...
            if($type == 'SELECT' || $type == 'RADIO' || $type == 'CHECKBOX' || $type == 'CLR') {
...
function atr_get_attribute_names($attribute_id, $attribute_value_id = -1) {
  ...
  } else if($type == 'RADIO' || $type == 'CHECKBOX' || $type == 'CLR') {
...
function atr_generate_post_elem($a, $item_id, $linked = false) {
  ...
  if($type == 'DIVIDER') {
    $html .= '<div class="control-group atr-type-' . strtolower($type) . '" id="atr-' . ($a['s_identifier'] <> '' ? $a['s_identifier'] : 'id' . $a['pk_i_id']) . '">';
    $html .= '<label class="control-label inp-group__title">&nbsp;</label>';
    $html .= '<strong class="atr-divider-label">' . $name . '</strong>';
    $html .= '</div>';
    return $html;
  }
  ...
  if($linked === false) {
    ...
    $html .= '<label class="control-label inp-group__title" for="atr' . $a['pk_i_id'] . '">' . ($name <> '' ? $name : __('New attribute', 'attributes')) . ($a['b_required'] == 1 ? '<span class="req">*</span>' : '') . '</label>';
    $html .= '<div class="controls">';
  } else {
    $html .= '<div class="atr-linked">';
  }
  ...
  if($a['s_type'] == 'SELECT') {
    ...
    if($item_atr['fk_i_attribute_value_id'] > 0) {
      ...
      if($linked === false) {
        if($linked === false) { 
          if($a['pk_i_id'] == 24){     
            $html .= '<option value="">' . __('Укажите источник', 'attributes') . '</option>'; 
          } 
          else {
            $html .= '<option value="">' . __('Выберите значение', 'attributes') . '</option>'; // исправлено
            foreach ($values as $value) {
              if($s_search_values_all == 1 && !in_array($value, $has_values)) {
                continue;
              }
    ...
    if(is_array($a['values']) && count($a['values']) > 0) {
      ...
      foreach($a['values'] as $v) {
        ...
        $checked = (@$item_atr_val['s_value'] == 1 ? 'checked' : '');
          ...
          if($a['s_identifier'] == 'Color_wig') {
            $icon_url = isset($v['s_image']) ? $v['s_image'] : 'default-icon.png';
            $icon_name = pathinfo($icon_url, PATHINFO_FILENAME);
            
              $html .= '<li class="clr_atr_li">
              <div class="atr-clr">
                <input type="radio" id="' . $idc . '" name="' . $idc . '" ' . $checked . '/> 
                  <label for="' . $idc . '"> <svg width="37" height="37" viewBox="0 0 40 40" fill="none" xmlns="http://www.w3.org/2000/svg"><circle cx="19.5" cy="19.5" r="17.5" fill=#' . $icon_name . ' stroke=#f8f7f7 stroke-width="2"/></svg>
                </div>
                    </label>
  ...
  } else if($a['s_type'] == 'CLR') {
    if(is_array($a['values']) && count($a['values']) > 0) {
      $html .= '<ul class="atr-ul-radio atr-ul-' . strtolower($a['s_type']) . '" style="padding-left: 0;">';
...
function atr_select_url() {
  ...
  if($attribute_id > 0) {
    ...
    if(is_array($values) && count($values) > 0) {
      ...
      $html .= '<option value="">' . __('Выберите персонажа', 'attributes') . '</option>';
