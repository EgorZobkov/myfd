...
(10, 'Place for Your banner #10', '<span class=\"php-tag\">&lt;?php</span><span class=\"php-function\"> osc_run_hook(<span class=\"php-function-data\">''rupayments_banner_10''</span>); </span><span class=\"php-tag\">?&gt;</span>', '0', '0', 0);";
        ...
        public function install() {
           ...
           $this->addban2();
            osc_set_preference('version', '411', 'rupayments', 'INTEGER');
	    ...
	    osc_set_preference('allow_after', '0', 'rupayments', 'BOOLEAN');
            ...
            osc_set_preference('yandex_enabled', '1', 'rupayments', 'BOOLEAN');
            ...
            osc_set_preference('interkassa_enabled', '1', 'rupayments', 'BOOLEAN');
			...
			osc_set_preference('robokassa_enabled', '1', 'rupayments', 'BOOLEAN');
			...
			osc_set_preference('wo_enabled', '1', 'rupayments', 'BOOLEAN');
            ...
            $sEnd = '<p>This is an automatic email, if you already did that, please ignore this email.</p><p>Thanks</p>';
           
            $description[osc_language()]['s_title'] = '{WEB_TITLE} - Publish option for your ad: {ITEM_TITLE}';
            $description[osc_language()]['s_text'] = 'We just published your item ({ITEM_TITLE}) on {WEB_TITLE}. <p>In order to make your ad available to anyone on {WEB_TITLE}, you should complete  the process and pay the publish fee. You could do that on the following link:  {EMAILPAYMENT_LINK}</p><p></p><p>You could make your ad colorized or/and (may be with discount)  premium and make it to appear on top result of the searches made on {WEB_TITLE}.  You could do that on the following link:   {EMAILPAYMENT_LINK}</p>'.$sEnd;
            $res = Page::newInstance()->insert(
                array('s_internal_name' => 'email_rupayments_offer_one', 'b_indelible' => '1'),
                $description
            );

            $description[osc_language()]['s_text'] = 'We just published your item ({ITEM_TITLE}) on {WEB_TITLE}.<p>Your payment is complited.</p> <p>You could mark your ad colorized or/and (may be with discount) premium and make it to appear on top result of the searches on {WEB_TITLE}.  You could do that on the following link:   {EMAILPAYMENT_LINK}</p>'.$sEnd;
            $res = Page::newInstance()->insert(
                array('s_internal_name' => 'email_rupayments_offer_two_not_allow_after', 'b_indelible' => '1'),
                $description
            );

            $description[osc_language()]['s_text'] = 'We just published your item ({ITEM_TITLE}) on {WEB_TITLE}.<p>You could mark your ad colorized or/and (may be with discount) premium and make it to appear on top result of the searches on {WEB_TITLE}.  You could do that on the following link:   {EMAILPAYMENT_LINK}</p>'.$sEnd;
            $res = Page::newInstance()->insert(
                array('s_internal_name' => 'email_rupayments_offer_two', 'b_indelible' => '1'),
                $description
            );

            $description[osc_language()]['s_text'] = '<p>Your payment is complited.</p>You could mark your ad colorized or/and (may be with discount) premium and make it to appear on top result of the searches on {WEB_TITLE}. You could do that on the following link:   {EMAILPAYMENT_LINK}'.$sEnd;
            $res = Page::newInstance()->insert(
                array('s_internal_name' => 'email_rupayments_offer_three_not_allow_after', 'b_indelible' => '1'),
                $description
            );

            $description[osc_language()]['s_text'] = 'You could mark your ad colorized or/and (may be with discount) premium and make it to appear on top result of the searches on {WEB_TITLE}.You could do that on the following link:   {EMAILPAYMENT_LINK}'.$sEnd;
            $res = Page::newInstance()->insert(
                array('s_internal_name' => 'email_rupayments_offer_three', 'b_indelible' => '1'),
                $description
            );

            $description[osc_language()]['s_text'] = 'For your ads operates Premium service and it will be still valid {DAYS_BEFOR_DEADLINE} days.<p>You may continue to mark your ad Premium (or choose another service) and make it to appear on top result of the searches  on {WEB_TITLE}. You could do that on the following link:   {EMAILPAYMENT_LINK}</p>'.$sEnd;
            $res = Page::newInstance()->insert(
                array('s_internal_name' => 'email_rupayments_alert_Premium', 'b_indelible' => '1'),
                $description
            );

            $description[osc_language()]['s_text'] = 'For your ads operates Colorized service and it will be still valid {DAYS_BEFOR_DEADLINE} days.<p>You may continue to mark your ad Premium (or choose another service) and make it to appear on top result of the searches  on {WEB_TITLE}. You could do that on the following link:   {EMAILPAYMENT_LINK}</p>'.$sEnd;
            $res = Page::newInstance()->insert(
                array('s_internal_name' => 'email_rupayments_alert_Colorized', 'b_indelible' => '1'),
                $description
            );

            $description[osc_language()]['s_text'] = 'For your ads operates Premium and Colorized service and it will be still valid {DAYS_BEFOR_DEADLINE} days.<p>You may continue to mark your ad Premium (or choose another service) and make it to appear on top result of the searches on {WEB_TITLE}. You could do that on the following link:   {EMAILPAYMENT_LINK}</p>'.$sEnd;
            $res = Page::newInstance()->insert(
                array('s_internal_name' => 'email_rupayments_alert_Primium_and_Colorized', 'b_indelible' => '1'),
                $description
            );
			
			$description[osc_language()]['s_text'] = 'Your ad expires and it will be still valid {DAYS_BEFOR_DEADLINE} days.<p>You may Renew your ad in {WEB_TITLE}. You could do that on the following link:   {EMAILPAYMENT_LINK}</p>'.$sEnd;
            $res = Page::newInstance()->insert(
                array('s_internal_name' => 'email_rupayments_alert_renew', 'b_indelible' => '1'),
                $description
            );

            $description[osc_language()]['s_text'] = 'Admininstrator just Published your item ({ITEM_TITLE}) on {WEB_TITLE}.'.$sEnd;
            $res = Page::newInstance()->insert(
                array('s_internal_name' => 'email_rupayments_admin_made_Published', 'b_indelible' => '1'),
                $description
            );

            $description[osc_language()]['s_text'] = 'Admininstrator just mark Premium your item ({ITEM_TITLE}) on {WEB_TITLE}.'.$sEnd;
            $res = Page::newInstance()->insert(
                array('s_internal_name' => 'email_rupayments_admin_made_Premium', 'b_indelible' => '1'),
                $description
            );

            $description[osc_language()]['s_text'] = 'Admininstrator just mark Premium and Colorized your item ({ITEM_TITLE}) on {WEB_TITLE}.'.$sEnd;
            $res = Page::newInstance()->insert(
                array('s_internal_name' => 'email_rupayments_admin_made_Premium_Colorized', 'b_indelible' => '1'),
                $description
            );

            $description[osc_language()]['s_text'] = 'Admininstrator just mark Colorized your item ({ITEM_TITLE}) on {WEB_TITLE}.'.$sEnd;
            $res = Page::newInstance()->insert(
                array('s_internal_name' => 'email_rupayments_admin_made_Colorized', 'b_indelible' => '1'),
                $description
            );

            $description[osc_language()]['s_text'] = 'Admininstrator just moved to Top your item ({ITEM_TITLE}) on {WEB_TITLE}.'.$sEnd;
            $res = Page::newInstance()->insert(
                array('s_internal_name' => 'email_rupayments_admin_moved_to_Top', 'b_indelible' => '1'),
                $description
            );
			
			$description[osc_language()]['s_text'] = 'Admininstrator just Renew your item ({ITEM_TITLE}) on {WEB_TITLE}.'.$sEnd;
            $res = Page::newInstance()->insert(
                array('s_internal_name' => 'email_rupayments_admin_renew', 'b_indelible' => '1'),
                $description
            );

            $description[osc_language()]['s_text'] = '<p>Admininstrator just filled up Credit Pack #1 your Wallet on {WEB_TITLE}.</p>'.$sEnd; 
            $res = Page::newInstance()->insert(
                array('s_internal_name' => 'email_rupayments_admin_filled_up_Credit_Pack_1', 'b_indelible' => '1'),
                $description
            );

            $description[osc_language()]['s_text'] = '<p>Admininstrator just filled up Credit Pack #2 your Wallet on {WEB_TITLE}.</p>'.$sEnd; 
            $res = Page::newInstance()->insert(
                array('s_internal_name' => 'email_rupayments_admin_filled_up_Credit_Pack_2', 'b_indelible' => '1'),
                $description
            );

            $description[osc_language()]['s_text'] = '<p>Admininstrator just filled up Credit Pack #3 your Wallet on {WEB_TITLE}.</p>'.$sEnd; 
            $res = Page::newInstance()->insert(
            ...
            */
            ...
            $description[osc_language()]['s_text'] = '<p><strong>{CONTACT_NAME}</strong> just added a banner on the site: <strong>{WEB_TITLE}</strong>.</p>'.$sEnd; 
            $res = Page::newInstance()->insert(
            ...
            $description[osc_language()]['s_text'] = '<p>Hi, <strong>{CONTACT_NAME}</strong></p><p>The administrator has just accepted your banner on the site: <strong>{WEB_TITLE}</strong>.</p><p>You can pay for the publication of the banner by clicking on this link: {BANNER_PAY_URL}</p>'.$sEnd; 
            $res = Page::newInstance()->insert(
            ...
            $description[osc_language()]['s_text'] = '<p>Hi, <strong>{CONTACT_NAME}</strong></p><p>There is not enough money to display your banner.</p>'.$sEnd; 
            $res = Page::newInstance()->insert(
            ...
            $description[osc_language()]['s_text'] = '<p>Hi, <strong>{CONTACT_NAME}</strong></p><p>On the <strong>{WEB_TITLE}</strong>, the user paid for your product.</p>'.$sEnd; 
            $res = Page::newInstance()->insert(
            ...
            $description[osc_language()]['s_text'] = '<p>On your site: <strong>{WEB_TITLE}</strong> received payment for the goods.</p>'.$sEnd; 
            $res = Page::newInstance()->insert(
            ...
            $description[osc_language()]['s_text'] = '<p>Hi, <strong>{CONTACT_NAME}</strong></p><p>Your Membership Plan expires and it will be still valid {DAYS_BEFOR_DEADLINE} days.<p>You may extend the time of your Membership Plan in <strong>{WEB_TITLE}</strong>. You could do that on the following link:   {MEMBERSHIP_PAY_LINK}</p>'.$sEnd; 
            $res = Page::newInstance()->insert(
        ...
        public function versionUpdate() {
            ...
            if ( $version < 320 ) {
			 ...
			 if ( $version < 361 ) {
				...
				$description[osc_language()]['s_text'] = 'Admininstrator just Renew your item ({ITEM_TITLE}) on {WEB_TITLE}.';
                $res = Page::newInstance()->insert(
                array('s_internal_name' => 'email_rupayments_admin_renew', 'b_indelible' => '1'),
                $description
                 );
				 $description[osc_language()]['s_title'] = '{WEB_TITLE} - Your ad expires!';
				 $description[osc_language()]['s_text'] = 'Your ad expires and it will be still valid {DAYS_BEFOR_DEADLINE} days.<p>You may Renew your ad in {WEB_TITLE}. You could do that on the following link:   {EMAILPAYMENT_LINK}</p>';
                  $res = Page::newInstance()->insert(
                array('s_internal_name' => 'email_rupayments_alert_renew', 'b_indelible' => '1'),
                $description
                 );
            ...
            $description[osc_language()]['s_title'] = '{WEB_TITLE} - Membership Plan expires!';
            $description[osc_language()]['s_text'] = '<p>Hi, <strong>{CONTACT_NAME}</strong></p><p>Your Membership Plan expires and it will be still valid {DAYS_BEFOR_DEADLINE} days.<p>You may extend the time of your Membership Plan in <strong>{WEB_TITLE}</strong>. You could do that on the following link:   {MEMBERSHIP_PAY_LINK}</p>'; 
            $res = Page::newInstance()->insert(
                array('s_internal_name' => 'email_rupayments_membership_expired', 'b_indelible' => '1'),
                $description
            );
	...
	public function get_class_color($itemId){
        ...
        public function getPublishPrice ( $categoryId, $item = false ) {
            ...
            if($result->numRows() || $city) {
                ...
                if($item) {
                    if(!is_null($cat_price) && $cat_price <= $city_price) {
                        $est_price = $cat_price;
                    }
                    else {
                        $est_price = $city_price;
                    }
                }
                else {
                    $est_price = $cat_price;
        ...
        */
        ...
        public function setCronUserBonus() {
            ...
            if(osc_get_preference('allow_periodbonus', 'rupayments')) {
                ...
                $mail_title = $site_title . ' - You have received a bonus!';
                $sEnd = '<p>This is an automatic email, if you already did that, please ignore this email.</p><p>Thanks</p>';
                ...
                if($interval->format('%a') % $period == 0 && osc_get_preference('periodbonus_last_accrual', 'rupayments') != date('Y-m-d', time())) {
                    ...
                    if($users) {
                        ...
                        foreach($users as $user) {                     
                            ...
                            $mail_text = "<p>Hi, {$user[s_name]}</p><p>You have received a bonus {$bonus_amount} {$currency} in your wallet. <p>Log in to your account and use one of the premium services on site {$site_title}</p>" . $sEnd;
            ...
            if($result->numRows()) {
                ...
                foreach($result as $item) {
                    ...
                    if(osc_rewrite_enabled()) {
                        $payLink = '/rupayments/banner-pay/' . $item['fk_i_banner_id'];
                        $deleteLink = '/rupayments/banner-delete/' . $item['fk_i_banner_id'];
                    }
                    else {
