...
class ItemForm extends Form {
  ...
  */
  ...
  public static function city_text2($item = null) {
    if ($item == null) { 
        $item = osc_item(); 
    }
    if (Session::newInstance()->_getForm('city') != '') {
    ...
    echo '<input type="text" id="sCity" name="city" value="' . (isset($item['s_city']) ? $item['s_city'] : '') . '" placeholder="Введите город" autocomplete="off">';
        // Скрытое поле для ID города
    echo '<input type="hidden" name="cityId" value="' . (isset($item['fk_i_city_id']) && $item['fk_i_city_id'] != null ? $item['fk_i_city_id'] : '') . '">';
    echo '<input type="hidden" name="region" value="' . (isset($item['s_region']) && $item['s_region'] != null ? $item['s_region'] : '') . '">';
    echo '<input type="hidden" name="regionId" value="' . (isset($item['fk_i_region_id']) && $item['fk_i_region_id'] != null ? $item['fk_i_region_id'] : '') . '">';
